...
function generate_graphs_new_naming(seed=22)
    ...
    for dataset_name in [
        ...
        # make axis for roc curve plots
        fig = Plots.plot(title="$(dataset_printing_name) ROC curves", legend=true,
                        xlabel="False positive rate", ylabel="True positive rate",
        ...
        fig = Plots.plot(title="$(dataset_printing_name) accuracies",
                         legend=true,
                         ylim=(0, 2))
        ...
        function margin_measures_bar_graph()
            ...
            fig = Plots.plot(ylabel="Mean margin on training points", xlabel="Approaches", title="Margins, $dataset_printing_name", legend=false)
        ...
        if dataset.feature_count == 2
            ...
            function graph_decision_boundaries(model, approach_number, validation_samples, validation_labels)
                ...
                ccol = cgrad([RGB(1,.2,.2), RGB(.2,.2,1)])
                # create first decision boundary plot.
                fig = Plots.plot(leg=:none, title="$(dataset_printing_name) approach $approach_number",
                                lims=(-1.2, 1.2), xlabel="Feature 1", ylabel="Feature 2", aspect_ratio=:equal)
                # draw contour
                contour!(axis_interval, axis_interval, (x,y)->z_matrix[point_to_index(x),point_to_index(y)],
                        f=true, nlev=2, c=ccol)
                # draw training set points
                ...
                # second, plot decision boundary for validation points
                fig2 = Plots.plot(leg=:none, title="$(dataset_printing_name) approach $approach_number, validation",
                                lims=(-1.2, 1.2), xlabel="Feature 1", ylabel="Feature 2", aspect_ratio=:equal)
                # draw contour again
                contour!(axis_interval, axis_interval, (x,y)->z_matrix[point_to_index(x),point_to_index(y)],
                        f=true, nlev=2, c=ccol)
